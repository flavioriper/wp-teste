...
final class WPBE_BULK extends WPBE_EXT {
    ...
    public function __construct() {
        ...
        //tabs
        $this->add_tab($this->slug, 'top_panel', esc_html('Bulk Edit', 'bulk-editor'));
    ...
    public function wpbe_ext_scripts() {
        ...
        <script>
            ...
            lang.<?php echo $this->slug ?>.want_to_delete = "<?php printf(esc_html('Sure? Delete [%s]?', 'bulk-editor'), $WPBE->settings->current_post_type) ?>";
            lang.<?php echo $this->slug ?>.deleting = "<?php esc_html_e('Bulk deleting', 'bulk-editor') ?>";
            lang.<?php echo $this->slug ?>.deleted = "<?php printf(esc_html('[%s](s) deleted!', 'bulk-editor'), $WPBE->settings->current_post_type) ?>";
            lang.<?php echo $this->slug ?>.bulking = "<?php esc_html_e('Bulk editing', 'bulk-editor') ?> ...";
            lang.<?php echo $this->slug ?>.bulked = "<?php printf(esc_html('[%s](s) edited! Table redrawing ...', 'bulk-editor'), $WPBE->settings->current_post_type) ?>";
            lang.<?php echo $this->slug ?>.bulked2 = "<?php printf(esc_html('[%s](s) edited!', 'bulk-editor'), $WPBE->settings->current_post_type) ?>";
    ...
    public function wpbe_tools_panel_buttons_end() {
        ...
        ?>
        &nbsp;|&nbsp;<span>
            <?php echo WPBE_HELPER::draw_advanced_switcher(0, 'wpbe_bind_editing', '', array('true' => esc_html('binded editing', 'bulk-editor'), 'false' => esc_html('binded editing', 'bulk-editor')), array('true' => 1, 'false' => 0), 'js_check_wpbe_bind_editing', 'wpbe_bind_editing'); ?>
            ...
            if ($WPBE->show_notes) {
                ...
                if (!empty($fields)) {
                    ...
                    $bind_tooltip = sprintf(esc_html('In FREE version of the plugin you can change only next fields: %s', 'bulk-editor'), implode(', ', $bind_tooltip));
                }
            }
            ?>

            <?php echo WPBE_HELPER::draw_tooltip(sprintf(esc_html('In this mode to the all selected [%s](s) will be set the value of a [%s] field which been edited', 'bulk-editor'), $WPBE->settings->current_post_type, $WPBE->settings->current_post_type) . '. ' . $bind_tooltip) ?>
            ...
            ?><?php echo WPBE_HELPER::draw_tooltip(esc_html('Select post type you want to manage', 'bulk-editor')) ?>
    ...
    private function init_bulk_keys() {
        ...
        $this->text_keys = array(
            ...
            'post_password' => array(
                'title' => esc_html('password', 'bulk-editor'),
        ...
        $this->other_keys = array(
            'post_status' => array(
                'title' => esc_html('post status', 'bulk-editor'),
        ...
        $this->other_keys['sticky_posts'] = array(
            'title' => esc_html('sticky posts', 'bulk-editor'),
        ...
        $options2 = array(
            'invalue' => esc_html('increase by value', 'bulk-editor'),
            'devalue' => esc_html('decrease by value', 'bulk-editor'),
            'delete' => esc_html('delete', 'bulk-editor'),
            'new' => esc_html('set new', 'bulk-editor')
        );
