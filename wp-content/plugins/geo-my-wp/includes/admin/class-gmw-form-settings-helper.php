...
class GMW_Form_Settings_Helper {
    ...
    /**
	 * Search form carousel image
	 *
	 * @param  [type] $value     [description].
	 *
	 * @param  [type] $name_attr [description].
	 */
	public static function carousel_image( $value, $name_attr ) {
		...
		<p>
			<label>
				<input
					type="checkbox"
					onclick="jQuery( this ).closest( 'td' ).find( '.carousel-image-options' ).slideToggle();"
					name="<?php echo $name_attr . '[enabled]'; // WPCS: XSS ok. ?>"
					value="1"
					<?php checked( 1, $value['enabled'] ); ?>
				/>
		...
		<div class="carousel-image-options gmw-options-box" <?php echo empty( $value['enabled'] ) ? 'style="display:none";' : ''; ?>>
			...
			<div class="single-option">
				<div class="option-content">
                    <label>
                        <?php esc_html_e( 'Disable Loop?', 'geo-my-wp' ); ?>
                        <input
                            type="checkbox"
                            size="5"
                            name="<?php echo $name_attr . '[loop]'; // WPCS: XSS ok. ?>"
                            value="<?php echo ! empty( $value['loop'] ) ? esc_attr( $value['loop'] ) : 'no'; ?>"
                            placeholder="Numeric value"
                        />
                    </label>
				</div>
			...
			<div class="single-option">
				<div class="option-content">
				<label>
                    <?php esc_html_e( 'Dots', 'geo-my-wp' ); ?>
					<input
						type="checkbox"
						size="5"
						name="<?php echo $name_attr . '[dots]'; // WPCS: XSS ok. ?>"
						value="<?php echo ! empty( $value['dots'] ) ? esc_attr( $value['dots'] ) : 'no'; ?>"
						placeholder="Numeric value"
					/>
                </label>
				</div>
            ...
            <div class="single-option">
                <div class="option-content">
                    <label>
                        <?php esc_html_e('Autoplay', 'geo-my-wp'); ?>
                        <input
                            type="checkbox"
                            size="5"
                            onclick="jQuery( this ).closest( 'td' ).find( '.autoplay-timeout' ).slideToggle();"
                            name="<?php echo $name_attr . '[autoplay]'; // WPCS: XSS ok. ?>"
                            value="<?php echo !empty($value['autoplay']) ? esc_attr($value['autoplay']) : '5000'; ?>"
                            placeholder="Numeric value"
                        />
                    </label>
                </div>
            </div>
			<div class="single-option autoplay-timeout" <?php echo empty( $value['autoplay'] ) ? 'style="display:none";' : ''; ?>>
				<label><?php esc_html_e( 'Autoplay Timeout', 'geo-my-wp' ); ?></label>
				<div class="option-content">
					<input
						type="text"
						size="5"
						name="<?php echo $name_attr . '[autoplay_time]'; // WPCS: XSS ok. ?>"
						value="<?php echo ! empty( $value['autoplay_time'] ) ? esc_attr( $value['autoplay_time'] ) : '5000'; ?>"
						placeholder="Numeric value"
					/>
				</div>
			</div>
