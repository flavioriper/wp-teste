 ...
 *
 */
function gmw_get_the_terms($taxonomy, int $post_id = 0 ) {
...
function gmw_get_post_taxonomies_terms_list( $post, $args = array() ) {
    ...
    # Preço do Imóvel - 11/08
	$output = $label_price ='';
    $valor_imovel = 0;
    $values = [];
    $termOferta = gmw_get_the_terms('oferta', $post->ID );
    $ofertaTaxonomy = [];
    $labelOfertas = array(
        'lancamento' => 'A partir de:',
        'venda' => 'Valor:',
        'temporada' => 'Diária por:',
        'aluguel' => 'Mensal por:',
    );
	...
	foreach ( $taxonomies as $taxonomy ) {
        ...
        $terms = gmw_get_the_terms( $taxonomy->name ,$post->ID);
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
            ...
            if($taxonomy->rewrite['slug'] <> 'faixa_preco') {
    ...
    if (!empty($termOferta)){
        foreach ($termOferta as $oferta){
            $ofertaTaxonomy[$oferta->slug] = $oferta;
            $label_price = $labelOfertas[$ofertaTaxonomy[$oferta->slug]->slug];
        }
    }
    if (!array_key_exists("lancamento", $ofertaTaxonomy)) {
        $valor_imovel = get_post_meta($post->ID, 'valor_do_imovel');
        if (!empty($valor_imovel)) {
            $valor_imovel = $valor_imovel[0];
        }
    } else {
        $fields = get_field_objects($post->ID);
        if(isset($fields['unidades']) && !empty($fields['unidades'])){
            if (isset($fields['unidades']['value']) && !empty($fields['unidades']['value'])){
                foreach ($fields['unidades']['value'] as $unidade){
                    $valueConverted = convertExtensionNumber($unidade['valor_da_unidade']);
                    $values[$valueConverted] = ['label' => $unidade['valor_da_unidade'], 'value' => $valueConverted];
                }
                $valor_imovel = $values[min(array_column($values, 'value'))]['label'];
                $label_price = $labelOfertas['lancamento'];
            }
        }
    }
    if ($valor_imovel && $label_price) {
        $output .= '<div class="gmw-taxonomy-terms gmw-taxes valor_do_imovel' . esc_attr($args['class']) . '">';
        $output .= '<span class="label">Valor do Imóvel:</span>';
        $output .= '<span class="gmw-terms-wrapper">';
        $output .= '<a><sup class="sup">' .$label_price. '</sup> R$' . $valor_imovel . '</a>';
        $output .= '</span>';
        $output .= '</div>';
    }
...
function convertExtensionNumber($string) {
    ...
    $singular = array(
        ...
        "milhão" => "000000",
        "bilhão" => "000000000",
        "trilhão" => "000000000000",
        "quatrilhão" => "000000000000000"
    );
    $plural = array(
        "centavos" => ".00",
        "reais" => "00",
        ...
        "milhões" => "000000",
        "bilhões" => "000000000",
        "trilhões"  => "000000000000",
        "quatrilhões" => "000000000000000"
    );

    if ($value[0] > 1) {
        $newValue = $value[0].$plural[$value[1]];
    } else {
        $newValue = $value[0].$singular[$value[1]];
    }
