...
class WPUF_Admin_Coupon {
    ...
    function update_coupon_meta( $post_id, $post ) {
        ...
        update_post_meta( $post_id, '_start_date', wpuf_date2mysql( ! empty( $post['start_date'] ) ? $post['start_date'] : '' ) );
        update_post_meta( $post_id, '_end_date', wpuf_date2mysql( ! empty( $post['end_date'] ) ? $post['end_date'] : '' ) );
